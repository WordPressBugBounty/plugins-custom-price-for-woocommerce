...
namespace CPWFreeVendor\WPDesk\Dashboard;

final class DashboardWidget
{
    const ID = 'custom-price-for-woocommerce';
    ...
    {
        return __('Grow your business with WP Desk', 'custom-price-for-woocommerce');
    }
    ...
    private function show_plugins(array $plugins, bool $for_free = \false)
    {
        ...
        foreach ($plugins as $plugin) {
            ...
            $add_to_cart_button_label = esc_html__('Buy now', 'custom-price-for-woocommerce');
            if ($plugin['add_to_cart_id'] === self::WP_DESK_CARE_ID) {
                $add_to_cart_button_label = esc_html__('Learn more', 'custom-price-for-woocommerce');
                $add_to_cart_url = $plugin_url;
            } else if ($plugin['add_to_cart_id'] === self::AUTOPAY_ID) {
                $add_to_cart_button_label = esc_html__('Download', 'custom-price-for-woocommerce');
            ...
            if ($for_free) {
                echo '<a class="button button-primary" href="' . \esc_url($plugin_url) . '" target="_blank">' . esc_html__('More info', 'custom-price-for-woocommerce') . '</a>';
