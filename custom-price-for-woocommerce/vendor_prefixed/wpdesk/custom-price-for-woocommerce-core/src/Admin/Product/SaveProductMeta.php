...
class SaveProductMeta implements Hookable
{
    /**
    ...
    public function save_product_meta($product)
    {
        ...
        $suggested = '';
        $minimum = '';
        if (isset($_POST['_cpw']) && in_array($product->get_type(), Helper::get_simple_supported_types())) {
        ...
        if (Integration::is_super()) {
            ...
            if ($product->is_type('subscription') && isset($_POST['_suggested_billing_period']) && array_key_exists(sanitize_key($_POST['_suggested_billing_period']), Helper::get_subscription_period_strings())) {
            ...
            if ($product->is_type('subscription') && $minimum && isset($_POST['_minimum_billing_period']) && array_key_exists(sanitize_key($_POST['_minimum_billing_period']), Helper::get_subscription_period_strings())) {
