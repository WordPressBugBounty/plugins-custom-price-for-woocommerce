...
use WP_Post;
class ProductFields implements Hookable
{
    ...
    public function add_to_metabox()
    {
        ...
        $show_billing_period_options = apply_filters('wc_cpw_supports_variable_billing_period', Helper::is_billing_period_variable($product_object));
        ?>
    ...
    public function get_product_fields($product_object, $show_billing_period_options, $loop = \false)
    {
        ...
        woocommerce_wp_select(['id' => is_int($loop) ? "variation_suggested_price_type[{$loop}]" : '_suggested_price_type', 'wrapper_class' => is_int($loop) ? 'form-row form-row-first' : '', 'value' => $product_object->get_meta('_suggested_price_type', \true), 'data_type' => 'price', 'label' => __('Suggested Price', 'custom-price-for-woocommerce'), 'options' => ['0' => __('Disabled', 'custom-price-for-woocommerce'), '1' => __('Show as description', 'custom-price-for-woocommerce'), '2' => __('Set amount to input as value', 'custom-price-for-woocommerce'), '3' => __('Set amount to input as placeholder', 'custom-price-for-woocommerce')], 'custom_attributes' => Helper::get_custom_attributes(), 'default' => 'no']);
        woocommerce_wp_text_input(['id' => is_int($loop) ? "variation_suggested_price[{$loop}]" : '_suggested_price', 'class' => 'wc_input_price short', 'wrapper_class' => is_int($loop) ? 'form-row form-row-last' : '', 'label' => __('Suggested price amount', 'custom-price-for-woocommerce') . ' (' . get_woocommerce_currency_symbol() . ')', 'desc_tip' => 'true', 'description' => __('Price to replace the default price string.  Leave blank to not suggest a price.', 'custom-price-for-woocommerce'), 'data_type' => 'price', 'custom_attributes' => Helper::get_custom_attributes(), 'value' => $product_object->get_meta('_suggested_price', \true)]);
        if (class_exists('WC_Subscriptions') && $show_billing_period_options && \false === $loop) {
            woocommerce_wp_select(['id' => '_suggested_billing_period', 'label' => __('per', 'custom-price-for-woocommerce'), 'wrapper_class' => 'show_if_subscription', 'options' => Helper::get_subscription_period_strings(), 'custom_attributes' => Helper::get_custom_attributes()]);
        }
        echo '<div class="cpw-row-flex">';
        echo '<div class="cpw-col">';
        // Minimum Price.
        woocommerce_wp_text_input(['id' => is_int($loop) ? "variation_min_price[{$loop}]" : '_min_price', 'class' => 'wc_input_price short', 'wrapper_class' => is_int($loop) ? 'form-row form-row-first' : '', 'label' => __('Minimum price', 'custom-price-for-woocommerce') . ' (' . get_woocommerce_currency_symbol() . ')', 'desc_tip' => 'true', 'description' => __('Lowest acceptable price for product. Leave blank to not enforce a minimum. Must be less than or equal to the set suggested price.', 'custom-price-for-woocommerce'), 'data_type' => 'price', 'value' => $product_object->get_meta('_min_price', \true), 'custom_attributes' => Helper::get_custom_attributes()]);
        if (class_exists('WC_Subscriptions') && $show_billing_period_options && \false === $loop) {
            // Minimum Billing Period.
            woocommerce_wp_select(['id' => '_minimum_billing_period', 'label' => __('per', 'custom-price-for-woocommerce'), 'wrapper_class' => 'show_if_subscription', 'options' => Helper::get_subscription_period_strings(), 'custom_attributes' => Helper::get_custom_attributes()]);
        }
        echo '</div>';
        echo '<div class="cpw-col">';
        // Option to hide minimum price.
        woocommerce_wp_checkbox(['id' => is_int($loop) ? "variation_hide_cpw_minimum[{$loop}]" : '_hide_cpw_minimum', 'label' => __('Hide minimum price', 'custom-price-for-woocommerce'), 'wrapper_class' => is_int($loop) ? 'form-row form-row-last form-input-checkbox' : '', 'description' => __('Option to not show the minimum price on the front end.', 'custom-price-for-woocommerce'), 'value' => $product_object->get_meta('_hide_cpw_minimum', \true), 'desc_tip' => \true, 'custom_attributes' => Helper::get_custom_attributes()]);
        echo '</div>';
        echo '</div>';
        echo '<div class="cpw-row-flex">';
        echo '<div class="cpw-col">';
        // Maximum Price.
        woocommerce_wp_text_input(['id' => is_int($loop) ? "variation_maximum_price[{$loop}]" : '_maximum_price', 'class' => 'wc_input_price short', 'wrapper_class' => is_int($loop) ? 'form-row form-row-first' : '', 'label' => __('Maximum price', 'custom-price-for-woocommerce') . ' (' . get_woocommerce_currency_symbol() . ')', 'desc_tip' => 'true', 'description' => __('Highest acceptable price for product. Leave blank to not enforce a maximum.', 'custom-price-for-woocommerce'), 'data_type' => 'price', 'value' => $product_object->get_meta('_maximum_price', \true), 'custom_attributes' => Helper::get_custom_attributes()]);
        echo '</div>';
        echo '<div class="cpw-col">';
        // Option to hide minimum price.
        woocommerce_wp_checkbox(['id' => is_int($loop) ? "variation_hide_cpw_maximum[{$loop}]" : '_hide_cpw_maximum', 'label' => __('Hide maximum price', 'custom-price-for-woocommerce'), 'wrapper_class' => is_int($loop) ? 'form-row form-row-last form-input-checkbox' : '', 'description' => __('Option to not show the maximum price on the front end.', 'custom-price-for-woocommerce'), 'value' => $product_object->get_meta('_hide_cpw_maximum', \true), 'desc_tip' => \true, 'custom_attributes' => Helper::get_custom_attributes()]);
        echo '</div>';
        echo '</div>';
    }
}
