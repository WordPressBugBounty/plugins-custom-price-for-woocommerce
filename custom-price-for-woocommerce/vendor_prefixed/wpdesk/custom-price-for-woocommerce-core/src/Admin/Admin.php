...
class Admin implements Hookable
{
    ...
    {
        if (Helper::is_cpw($product) && !isset($product->is_filtered_price_html)) {
            $price = wc_get_price_html_from_text() . Helper::get_price_string($product, 'minimum', \true);
        } elseif (Helper::has_cpw($product) && !isset($product->is_filtered_price_html)) {
            $price = wc_get_price_html_from_text() . Helper::get_price_string($product, 'minimum-variation', \true);
        }
        return $price;
    }
    /**
    ...
    public function column_display($column_name, $post_id)
    {
        switch ($column_name) {
            case 'price':
                ...
                $cpw = Helper::is_cpw($_product) ? 'yes' : 'no';
                // If variable billing is enabled, continue to show options. Otherwise, deprecate.
                $is_sub = wc_bool_to_string(Helper::is_subscription($_product));
                $is_variable_billing = Helper::is_billing_period_variable($_product);
                ...
                $suggested = wc_format_localized_price(Helper::get_suggested_price($_product));
                $suggested_period = Helper::get_suggested_billing_period($_product);
                $min = wc_format_localized_price(Helper::get_minimum_price($_product));
                $min_period = Helper::get_minimum_billing_period($_product);
                $max = wc_format_localized_price(Helper::get_maximum_price($_product));
                $is_cpw_allowed = wc_bool_to_string($_product->is_type(Helper::get_simple_supported_types()));
                $is_minimum_hidden = wc_bool_to_string(Helper::is_minimum_hidden($_product));
                echo '
