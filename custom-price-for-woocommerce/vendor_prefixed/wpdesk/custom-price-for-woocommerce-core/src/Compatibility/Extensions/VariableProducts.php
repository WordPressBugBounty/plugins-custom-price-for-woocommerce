 ...
 **/
class VariableProducts implements Hookable
{
    /**
    ...
    public function variable_sync_has_cpw_status(WC_Product $product)
    {
        ...
        if ($product->is_type(Helper::get_variable_supported_types())) {
            global $wpdb;
            ...
            if (0 < $cpw_variation_count) {
                ...
                if ($min_variation && Helper::is_cpw($min_variation)) {
                    if (\false === Helper::get_minimum_price($min_variation) || Helper::is_minimum_hidden($min_variation)) {
