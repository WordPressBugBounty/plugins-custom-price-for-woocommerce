 ...
 **/
class WCSubscriptions implements Hookable
{
    /**
    ...
    public function is_switchable($is_switchable, $product, $variation)
    {
        ...
        if (self::supports_cpw_switching() && Helper::is_cpw($_cpw_product)) {
            $is_switchable = \true;
        }
    ...
    public function add_switch_query_args($permalink, $subscription_id, $item_id)
    {
        ...
        if (Helper::is_cpw($cpw_product)) {
            $prefix = Helper::get_suffix($cpw_product);
            $args['cpw' . $prefix] = $subscription->get_item_subtotal($existing_item, $subscription->get_prices_include_tax());
            if (Helper::is_billing_period_variable($cpw_product)) {
                $args['cpw-period' . $prefix] = CoreCompatibility::get_prop($subscription, 'billing_period');
            }
    ...
    public function is_identical_product($is_identical, $product_id, $quantity, $variation_id, $subscription, $item)
    {
        if ($is_identical && self::supports_cpw_switching()) {
            ...
            if (Helper::is_cpw($cpw_id)) {
                $prefix = Helper::get_suffix($cpw_id);
                ...
                $new_subscription_price = floatval(Helper::get_posted_price($cpw_id, $prefix));
                $initial_subscription_period = CoreCompatibility::get_prop($subscription, 'billing_period');
                $new_subscription_period = Helper::get_posted_period($cpw_id, $prefix);
                // If variable billing period check both price and billing period.
                if (Helper::is_billing_period_variable($cpw_id) && $new_subscription_price === $initial_subscription_price && $new_subscription_period === $initial_subscription_period) {
